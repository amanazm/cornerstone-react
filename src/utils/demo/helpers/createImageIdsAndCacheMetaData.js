...
export default async function createImageIdsAndCacheMetaData({
  ...
  const auth = cornerstoneWADOImageLoader.configure({
  ...
  const IMAGE_ORDER = "00200013";
  const MODALITY = "00080060";
  const IMAGE_POSITION = "00200032";
  ...
  const newimageIdsOrder = () => {
    instances.sort(function (a, b) {
      return b[IMAGE_POSITION].Value[2] - a[IMAGE_POSITION].Value[2];
    });

    // while (instances.length === orderedIds.length) {}
  };
  newimageIdsOrder();
